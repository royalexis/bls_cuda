...
def transitplot_wchains(time,flux,sol,serr,chain,burnin,nplanetplot=1, \
        itime=-1, ntt=0, tobs=0, omc=0, dtype=0, koi_id=None, savefig='null',nintg=41):
    "plot the transit model"
    nplanet=int((len(sol)-8)/10) #number of planets
    ...
    tfit5.transitmodel(nplanet,sol2,time,itime,nttin,tobsin,omcin,tmodel,dtypein,nintg)
    ...
    tfit5.transitmodel(nplanet,sol2,time,itime,nttin,tobsin,omcin,tmodel2,dtypein,nintg)
    ...
    for ii in range(100):
        ...
        tfit5.transitmodel(nplanet,sol2,mctime,mcitime,nttin,tobsin,omcin,tmodel2,dtypein2,nintg)
...
def setup_gibbs():
    ...
    rmin=np.array([-11.5e0,-10.0 ,-10.0 ,0.0000,0.0000,0.000,-1.0e6,-1.0   ,-1.0e+5,1.0e-6, 0.0 ,-20.0 ,-1.000,-1.000,\
                   -1.0e6,-1.0e6,-1.0e6,-1.0e6, 0.0   , 0.0,    0.0])
    rmax=np.array([ 6.9e0 , 10.0 , 10.0 ,1.0000,1.0000,1.000, 1.0e6, 1.0   , 1.0e+5,1.0e+6,10.0 ,2.000 , 1.000, 1.000,\
...
def readarchcat(koipropsfile,koicat):
    ...
    for line in f:
        ...
        if(columns[0][0] != '#'):
            if(icount > 0):  #skip header info
                koicat.kid.append(int(columns[0]))
                koicat.koi.append(float(columns[0])+0.01)
                koicat.kepid.append(" ")
                
                #Get stellar values 
                rad    = float(columns[14])
                rad_e  = float(columns[15])
                mass   = float(columns[16])
                mass_e = float(columns[17])
                rhostar =  (mass * msun) / (4/3 * np.pi * np.power(rad * rsun,3)) / 1000 #mean stellar density cgs
                ...
                koicat.rdrsep.append(float(columns[8]))
                koicat.rdrsem.append(float(columns[9]))
                koicat.rstar.append(float(columns[14]))
                koicat.rstar_ep.append(float(columns[15]))
                koicat.rstar_em.append(-float(columns[15]))
                koicat.teff.append(float(columns[50]))
                koicat.teff_e.append(float(columns[51]))
                koicat.rhostar.append(rhostar)
                koicat.rhostar_ep.append(1.0)
                koicat.rhostar_em.append(-1.0)
                #if np.abs(float(columns[38]))> 1.0e-10:
                #    koicat.rhostar_em.append(float(columns[38]))
                #else:
                #    koicat.rhostar_em.append(float(-0.01))
...
def transitplot(time,flux,sol,nplanetplot=1, itime=-1, ntt=0, tobs=0, omc=0, dtype=0,nintg=41):
    ...
    if np.abs(y2-y1)<1.0e-10:
        y1=min(flux)
        y2=max(flux)
    plt.axis((-2*tdur,2*tdur,y1,y2)) #readjust range
    plt.show()  #show the plot
        
    return;
        

def transitmodel (sol,time, itime=-1, ntt=0, tobs=0, omc=0, dtype=0, nintg=41): 
    ...
    tmodel= zeros(len(time)) #contains the transit model
    tfit5.transitmodel(nplanet,sol,time,itime,nttin,tobsin,omcin,tmodel,dtypein,nintg)
    return tmodel;
    ...
    for line in f:
        ...
        if columns[0][0:3]=='RHO':
            ...
            if np1>nplanet:
                nplanet=np1
            j=int(10*(np1-1)+8+3)
            solin[j]=np.abs(float(columns[1]))
            serrin[j]=columns[3]
